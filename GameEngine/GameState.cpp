...
class GameState {
private:
    // Helper mapper block for backward component compatibility checks
...
public:
    std::pair<int, int> findKing(Color kingColor, const Board& board) const {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
    ...
    bool isCheck(Color kingColor, const Board& board) const {
        std::pair<int, int> kingPos = findKing(kingColor, board);
